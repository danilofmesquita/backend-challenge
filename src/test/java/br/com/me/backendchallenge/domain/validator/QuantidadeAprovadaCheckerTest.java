...
class QuantidadeAprovadaCheckerTest {

    Pedido pedido;
    QuantidadeAprovadaChecker checker;
    ...
    void setUp() {
        ...
        pedido.addItem(new ItemDTO(null, null, BigDecimal.valueOf(2L), 5L));
        checker = new QuantidadeAprovadaChecker();
    }
    ...
    void deveRetornarAprovadoMaior() {
        ...
        final var context = novaAlteracao(Status.APROVADO, 15L, new BigDecimal("20.0"));
        checker.doCheck(context);
        Assertions.assertThat(context.getOut().getStatus()).hasSize(1).contains(Status.APROVADO_QTD_A_MAIOR);
    }
    ...
    void deveRetornarAprovadoMenor() {
        final var statusPedidoMock = Mockito.mock(StatusPedido.class);
        this.pedido.setStatusAtual(statusPedidoMock);
        final var context = novaAlteracao(Status.APROVADO, 1L, new BigDecimal("20.0"));
        checker.doCheck(context);
        Assertions.assertThat(context.getOut().getStatus()).hasSize(1).contains(Status.APROVADO_QTD_A_MENOR);
    }
